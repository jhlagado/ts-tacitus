...
- Existing list ops implemented: `slots`, `cons`, `concat`, `drop-head`, `get-at`, `set-at`, `enlist` (mEnlist)
- Missing (per analysis): `length`, `head`, `tail`, `uncons`, `sort`, `bfind` (list + maplist variants), `find` (polymorphic), `get`, `set`, `mapsort`, `keys`, `values`, `hindex`, `hfind`, capsule system (parser, field offsets, method maplist, with combinator, .method sigil), address-level primitives if desired.
...
2. Implement `bfind ( maplist key bfind { kcmp } — addr | nil )` returning value element (not key) per spec; require pre-sorted order.
