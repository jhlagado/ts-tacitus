...
- Errors and NIL: Out-of-bounds address queries yield NIL; invalid `REF` payloads for fetch/store raise errors; window bounds guard access to globals, data stack, and return stack.
...
- Compound locals: slots store a `REF` (return-stack region) to the compound header; compatible compound assignment overwrites payload+header at that address without changing the slot’s reference.
