...
| • patch16(addr, offExit)       | offExit := exitAddr − (addr + 2)                                                        | [ …, savedRSP, NIL ]      | [ …, … ]                |
| drop skip                      | Remove NIL skip slot                                                                    | [ …, savedRSP ]           | [ …, … ]                |
...
- Constant compile-time meta‑arity
  ...
  - endcond closes any open when (if needed), then backpatches all recorded exits until RSP equals savedRSP; finally drops skip and savedRSP.
...
E) endcond (closer)
- Input: "cond endcond"
  - Behavior: no exits recorded; loop patches nothing; frame [ savedRSP, NIL ] torn down; exit performed
