# switch / case … of / default / endswitch — Guarded Multi-branch (Exploration Draft)
...
- Tokens (proposed): switch, case, of, default, endswitch
...
- Subject management is explicit:
  - Transient subject: duplicate (dup) wherever needed; drop after endswitch if it should not escape.
  - Local subject: use a var or other binding; no need to dup.

Behavior
- True predicate → enter case body; at the next boundary (case/default/endswitch) that case is closed and the construct exits (skips the rest).
- False predicate → jump over that case’s body to the next case or default; if none, falls to endswitch (no default).
- Boundaries are explicit tokens: case, default, endswitch.

Compiler model (constant meta-arity)

Legend (compile-time data stack; TOS rightmost)
- preExit: code address (number) immediately AFTER `Branch +3` (the opcode of the second Branch). Back‑branch target for taken cases (the “pre‑exit” hop). Kept at NOS until endswitch.
- p_exit: 16‑bit operand address (number) of the opener’s forward Branch (patched to the final exit at endswitch).
...
Case body compiles until the next boundary token: case, default, or endswitch.

4) Boundary auto-close (when encountering case/default/endswitch)
...
| begin default region | Compile code until endswitch            | [ …, p_exit, preExit, NIL ] |

6) endswitch (final closer)
...
- Constant compile-time meta‑arity
  ...
  - endswitch pops exactly one pending (should be NIL after auto-close), then tears down preExit and p_exit in fixed sequence.
- preExit discipline
...
- Single‑exit guarantee
  - All taken cases back‑branch to preExit, then the unified forward Branch at preExit jumps to a single exit set by endswitch.
...
- Nesting and LIFO
  ...
  - Inner immediates (case/of/default/endswitch) must restore their own frame before outer ones run.
- Default region
  - default is optional; when present, it runs until endswitch.
  - If all predicates are false and default is omitted, control reaches endswitch and exits.
...
- Input: "switch case P of B default D endswitch"
  - If pending was set, it is closed; stack returns to [ …, p_exit, preExit, NIL ]
  - D compiles; endswitch later patches p_exit

E) endswitch (final closer)
- Input: "switch endswitch"
  - Data stack becomes [ … ] (pop pending(NIL); drop preExit; patch p_exit; pop p_exit)
- Input: "switch case P of B endswitch"
  - Behavior: endswitch auto‑closes the open case (back‑branch + patch), then drops preExit, patches p_exit, pops p_exit
...
- Input:
  ```
  switch
    switch endswitch   \ inner switch inside outer default or case body
  endswitch
  ```
  - Inner endswitch runs first; outer endswitch runs last; outer frame intact

G) Behavioral (first‑true‑wins, default)
- "switch case 1 of 10 case 1 of 20 endswitch" → 10
- "switch case 0 of 10 case 1 of 20 endswitch" → 20
- "switch case 0 of 10 case 0 of 20 default 99 endswitch" → 99
...
- Pros:
  - Fewer distinct keywords to learn; `case/of/default/endswitch` mirrors common idioms.
...
- Diagnostics quality: “case without switch”, “of without case”, “endswitch with pending==NIL but unexpected state”, etc.
