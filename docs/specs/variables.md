...
The global heap is **managed by the GP register**, which points to the next available slot. Allocation proceeds using a **bump allocation strategy**—a simple mechanism where memory grows forward in contiguous 32-bit words. There is no compaction, reclamation, or reuse of intermediate space: once allocated, space is retained permanently unless the last item is explicitly popped (e.g., for rollback or shadowing).

### Heap as a Stack-Like Structure

Although the heap is not a stack in the traditional sense, it behaves like a **monotonic, append-only stack**. Values are added (pushed) as needed, but very rarely removed. There is no automatic mechanism for reclaiming heap space. Instead, values in the global heap are designed to **persist indefinitely**. This makes the heap suitable for storing stable references—unlike the data or return stacks, where values are ephemeral and routinely discarded.

### Global Availability and Lifetime

References to items in the global heap are assumed to be **global and persistent**. When a value is stored in the dictionary (such as a global variable or a function definition), the dictionary entry typically includes a reference to some payload stored in the heap. Once created, such a reference remains valid for the **entire lifetime of the program**. This differs from other stacks in Tacit (like the data or return stacks), where references are only valid during execution.
...
In all cases, the heap preserves structure and reference integrity. Once a compound structure is allocated, its memory layout is stable. This enables references to be freely shared across the runtime, embedded in capsules, or accessed via named dictionary entries.
