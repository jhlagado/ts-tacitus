...
* **Alignment with Multitasking:** Local variables must coexist cleanly with Tacit's multitasking model.
...
The lifetime of local variables is tied to the lifespan of the function call stack frame. When the function terminates, cleanup is performed by simply resetting the stack pointer to the base pointer. This single operation effectively frees all local variables and allocated buffers.
...
The Tacit task scheduler manages lightweight tasks by maintaining a list of active functions that yield control cooperatively. Tasks share a single large return stack, avoiding VM overhead. This design supports a lightweight model that focuses on efficiency and simplicity while supporting concurrency through controlled cooperative multitasking.
