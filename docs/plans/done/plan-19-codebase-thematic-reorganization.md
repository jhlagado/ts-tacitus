...
- `ops/math/arithmetic-ops.ts` and `ops/math/comparison-ops.ts` now import `VM`, `TacitWord`, and `areValuesEqual` via `@src/core`.
- `ops/control/branch-ops.ts` now imports `VM`, `TacitWord`, and tagged helpers via `@src/core`.
