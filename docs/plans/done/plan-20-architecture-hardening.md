...
- Strings→VM coupling: `strings/symbol-table.ts` uses `VM` where a type‑only `TacitWord` suffices.
...
1. In `strings/symbol-table.ts`, replace `VM` usage with `TacitWord` type where possible.
2. Ensure `strings/**` has no direct runtime coupling (type-only Core usage allowed).
3. Verify: full tests green.

Status: COMPLETED (Strings now uses TacitWord type; no VM import; tests green)
...
1. Introduce a single source of truth for builtin opcode → TacitWord mapping (e.g., `ops/dispatch.ts` or extend `builtins-register.ts`).
...
Status: COMPLETED (centralized opcode→TacitWord mapping inside builtins; behavior unchanged; interpreter tests pass)
...
1. Add import restrictions:
   - Core must not import from Lang or Ops.
   - Strings must not import `VM` (type-only `TacitWord` allowed).
...
- Strings depends on `TacitWord` type only; no `VM` import in `strings/**`.
