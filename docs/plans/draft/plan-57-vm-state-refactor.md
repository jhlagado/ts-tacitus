...
- Keep `tokenizer` at root (no parser group unless more fields emerge).
...
- Leave `tokenizer` at root; no parse group for now.
...
- If more parser state is added later, consider a `parse` sub-object; currently `tokenizer` stays flat.
