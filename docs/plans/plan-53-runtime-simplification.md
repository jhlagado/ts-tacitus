...
1. **`src/ops/builtins.ts`**
   - Remove unused `Verb` table entries (Nop, removed opcodes); collapse the `OPCODE_TO_VERB` table generation into a direct object literal with only active entries.
