...
## 1. Lists and Capsules

- The `span` tag is renamed to `list`. All lists are length-prefixed sequences of tagged values. This replaces previous terminology and unifies the representation.
- Lists may nest recursively, and the length field at the head is sufficient to identify the structure.
- Capsules remain a subtype of lists: the last slot is a function reference (`apply`, `next`, or another conventional word).
...
- The buffer tag will distinguish it from lists, and segment bits in the tag identify origin (stack, code, string, global).

## 3. Tagged Values

- Update the tag space:
  - Introduce segment identification bits (e.g. 2 bits) for stack/global distinction.
  - Explicit tag kinds: `int`, `float`, `list`, `buffer`, `ref`, `string`, `code`.

- Rules for runtime validation:
  - Stack references must include a valid BP (base pointer) check to ensure scope correctness.
  - Lists and buffers are self-identifying and include headers.
...
- Sequences, lists, and buffers may only be run or used when validated against their tag and segment constraints.
