...
import { NIL, createVM, VM } from '../../../core';
...
describe('Heap-backed dictionary ops', () => {
  ...
  test('define + lookup simple value', () => {
    const stack = executeTacitCode(vm, "42 'x define 'x lookup fetch");
    expect(stack).toEqual([42]);
  });

  test('define + lookup list value', () => {
    const stack = executeTacitCode(vm, "( 1 2 ) 'xs define 'xs lookup fetch length");
    expect(stack).toEqual([2]);
  });

  test('shadowing returns latest definition', () => {
    const stack = executeTacitCode(vm, "1 'a define 2 'a define 'a lookup fetch");
    expect(stack).toEqual([2]);
  });
  ...
  test('define rejects non-string name', () => {
    expect(() => executeTacitCode(vm, '42 99 define')).toThrow(/STRING name/);
  });

  test('lookup rejects non-string name', () => {
    expect(() => executeTacitCode(vm, '99 lookup')).toThrow(/STRING name/);
  });
});
