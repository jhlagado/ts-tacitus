...
describe('In-Place Compound Mutation', () => {
  ...
  describe('Basic In-Place Mutation', () => {
    test('should mutate empty list in place', () => {
      ...
      // Perform mutation
      mutateCompoundInPlace(vm, targetAddr, SEG_RSTACK, newHeader);
    ...
    test('should mutate single-element list in place', () => {
      ...
      resetVM();
      const newListResult = executeTacitCode('(42)');
      const newHeader = newListResult[newListResult.length - 1];
      
      // Setup existing single-element list at target location
      const targetAddr = 100;
      const existingHeader = toTaggedValue(1, Tag.LIST);
      vm.memory.writeFloat32(SEG_RSTACK, targetAddr, existingHeader);
      vm.memory.writeFloat32(SEG_RSTACK, targetAddr - BYTES_PER_ELEMENT, 999); // Old value
      ...
      const resultElement = vm.memory.readFloat32(SEG_RSTACK, targetAddr - BYTES_PER_ELEMENT);
    ...
    test('should mutate multi-element list in place', () => {
      ...
      resetVM();
      const newListResult = executeTacitCode('(1 2 3)');
      const newHeader = newListResult[newListResult.length - 1];
      ...
      vm.memory.writeFloat32(SEG_RSTACK, targetAddr, existingHeader);
      vm.memory.writeFloat32(SEG_RSTACK, targetAddr - 3 * BYTES_PER_ELEMENT, 999); // elem0 (old)
      vm.memory.writeFloat32(SEG_RSTACK, targetAddr - 2 * BYTES_PER_ELEMENT, 888); // elem1 (old)
      vm.memory.writeFloat32(SEG_RSTACK, targetAddr - 1 * BYTES_PER_ELEMENT, 777); // elem2 (old)
      ...
      const elem0 = vm.memory.readFloat32(SEG_RSTACK, targetAddr - 3 * BYTES_PER_ELEMENT);
      const elem1 = vm.memory.readFloat32(SEG_RSTACK, targetAddr - 2 * BYTES_PER_ELEMENT);
      const elem2 = vm.memory.readFloat32(SEG_RSTACK, targetAddr - 1 * BYTES_PER_ELEMENT);
  ...
  describe('Compatibility Enforcement', () => {
    test('should reject incompatible slot counts', () => {
      ...
      resetVM();
      const newListResult = executeTacitCode('(1 2 3)'); // LIST:3
      const newHeader = newListResult[newListResult.length - 1];
    ...
    test('should reject non-compound mutation attempts', () => {
      ...
      expect(() => {
        mutateCompoundInPlace(vm, targetAddr, SEG_RSTACK, 42);
  ...
  describe('Memory Layout Verification', () => {
    test('should preserve memory layout without RP advancement', () => {
      ...
      resetVM();
      const newListResult = executeTacitCode('(10 20)');
      const newHeader = newListResult[newListResult.length - 1];
      ...
      const elem0 = vm.memory.readFloat32(SEG_RSTACK, targetAddr - 2 * BYTES_PER_ELEMENT);
      const elem1 = vm.memory.readFloat32(SEG_RSTACK, targetAddr - 1 * BYTES_PER_ELEMENT);
  ...
  describe('Edge Cases', () => {
    test('should handle nested list mutation correctly', () => {
      ...
      resetVM();
      const nestedResult = executeTacitCode('(1 (2) 3)'); // LIST:4 total
      const nestedHeader = nestedResult[nestedResult.length - 1];
      ...
      mutateCompoundInPlace(vm, targetAddr, SEG_RSTACK, nestedHeader);
    ...
    test('should maintain data integrity during mutation', () => {
      ...
      resetVM();
      const validList = executeTacitCode('(100 200 300)');
      const validHeader = validList[validList.length - 1];
      ...
      vm.memory.writeFloat32(SEG_RSTACK, targetAddr - 3 * BYTES_PER_ELEMENT, 111);
      vm.memory.writeFloat32(SEG_RSTACK, targetAddr - 2 * BYTES_PER_ELEMENT, 222);
      vm.memory.writeFloat32(SEG_RSTACK, targetAddr - 1 * BYTES_PER_ELEMENT, 333);
      
      // Perform successful mutation
      mutateCompoundInPlace(vm, targetAddr, SEG_RSTACK, validHeader);
      
      // Verify all elements updated correctly
      // Stack order for (100 200 300) is [300, 200, 100, header]
      expect(vm.memory.readFloat32(SEG_RSTACK, targetAddr - 3 * BYTES_PER_ELEMENT)).toBe(300);
      expect(vm.memory.readFloat32(SEG_RSTACK, targetAddr - 2 * BYTES_PER_ELEMENT)).toBe(200);
      expect(vm.memory.readFloat32(SEG_RSTACK, targetAddr - 1 * BYTES_PER_ELEMENT)).toBe(100);
    });
  });
});