...
import { SEG_DATA, CELL_SIZE, RSTACK_BASE_BYTES, RSTACK_BASE_CELLS, STACK_BASE_CELLS } from '../../../core/constants';
...
describe('In-Place Compound Mutation', () => {
  ...
  describe('Basic In-Place Mutation', () => {
    test('should mutate empty list in place', () => {
      ...
      vm.memory.writeFloat32(SEG_DATA, RSTACK_BASE_BYTES + targetAddr, existingHeader);
      ...
      const resultHeader = vm.memory.readFloat32(SEG_DATA, RSTACK_BASE_BYTES + targetAddr);
    ...
    test('should mutate single-element list in place', () => {
      ...
      vm.memory.writeFloat32(SEG_DATA, RSTACK_BASE_BYTES + targetAddr, existingHeader);
      vm.memory.writeFloat32(SEG_DATA, RSTACK_BASE_BYTES + targetAddr - CELL_SIZE, 999); // Old value
      ...
      const resultElement = vm.memory.readFloat32(SEG_DATA, RSTACK_BASE_BYTES + targetAddr - CELL_SIZE);
    ...
    test('should mutate multi-element list in place', () => {
      ...
      vm.memory.writeFloat32(SEG_DATA, RSTACK_BASE_BYTES + targetAddr, existingHeader);
      vm.memory.writeFloat32(SEG_DATA, RSTACK_BASE_BYTES + targetAddr - 3 * CELL_SIZE, 999); // elem0 (old)
      vm.memory.writeFloat32(SEG_DATA, RSTACK_BASE_BYTES + targetAddr - 2 * CELL_SIZE, 888); // elem1 (old)
      vm.memory.writeFloat32(SEG_DATA, RSTACK_BASE_BYTES + targetAddr - 1 * CELL_SIZE, 777); // elem2 (old)
      ...
      const elem0 = vm.memory.readFloat32(SEG_DATA, RSTACK_BASE_BYTES + targetAddr - 3 * CELL_SIZE);
      const elem1 = vm.memory.readFloat32(SEG_DATA, RSTACK_BASE_BYTES + targetAddr - 2 * CELL_SIZE);
      const elem2 = vm.memory.readFloat32(SEG_DATA, RSTACK_BASE_BYTES + targetAddr - 1 * CELL_SIZE);
  ...
  describe('Memory Layout Verification', () => {
    test('should preserve memory layout without RP advancement', () => {
      ...
      const elem0 = vm.memory.readFloat32(SEG_DATA, RSTACK_BASE_BYTES + targetAddr - 2 * CELL_SIZE);
      const elem1 = vm.memory.readFloat32(SEG_DATA, RSTACK_BASE_BYTES + targetAddr - 1 * CELL_SIZE);
  ...
  describe('Edge Cases', () => {
    ...
    test('should maintain data integrity during mutation', () => {
      ...
      vm.memory.writeFloat32(SEG_DATA, RSTACK_BASE_BYTES + targetAddr - 3 * CELL_SIZE, 111);
      vm.memory.writeFloat32(SEG_DATA, RSTACK_BASE_BYTES + targetAddr - 2 * CELL_SIZE, 222);
      ...
      expect(vm.memory.readFloat32(SEG_DATA, RSTACK_BASE_BYTES + targetAddr - 3 * CELL_SIZE)).toBe(300);
      expect(vm.memory.readFloat32(SEG_DATA, RSTACK_BASE_BYTES + targetAddr - 2 * CELL_SIZE)).toBe(200);
      expect(vm.memory.readFloat32(SEG_DATA, RSTACK_BASE_BYTES + targetAddr - 1 * CELL_SIZE)).toBe(100);
    });
  });
});
