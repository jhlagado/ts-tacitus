...
describe('swap Operation', () => {
  ...
  describe('list operations', () => {
    ...
    test('should swap a value with a simple list', () => {
      ...
      const stack = vm.getStackData();
      expect(stack.length).toBe(6);
      expect(stack[0]).toBe(5); // Base element unchanged
      
      // List should now be in NOS position
      ...
      expect(stack[5]).toBe(10); // Value moved to TOS
    });
    ...
    test('should swap two simple lists', () => {
      ...
      // Second list should now be in NOS position
      ...
      // First list should now be on TOS
    ...
    test('should handle empty lists during swap', () => {
      ...
      // Empty list should now be in NOS position
      const swappedListTag = fromTaggedValue(stack[1]);
      expect(swappedListTag.tag).toBe(Tag.LIST);
      ...
      expect(stack[3]).toBe(42); // Value moved to TOS
    });
  });
