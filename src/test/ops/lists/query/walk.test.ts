...
describe('List query: walk', () => {
  ...
  test('walk simple flat list cells', () => {
    const out = captureTacitOutput(vm, `( 10 20 30 ) ref 0 walk raw walk raw walk raw`);
  ...
  test('walk out-of-range resets idx to 0 and yields NIL', () => {
    const out = captureTacitOutput(vm, `( 7 8 ) ref 2 walk swap raw`);
    expect(out).toEqual(['0']);
  });

  test('walk nested ref fetch materializes for pretty print', () => {
    const out = captureTacitOutput(vm, `( 1 ( 2 3 ) 4 ) ref 1 walk fetch .`);
