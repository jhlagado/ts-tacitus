...
describe('FunctionTable', () => {
  ...
  describe('registerBuiltin', () => {
    test('should register built-in functions at valid indices', () => {
      const testFn: OpcodeFunction = (vm: VM) => {
        vm.push(42);
      };
    ...
    test('should throw error for invalid indices', () => {
      ...
      expect(() => functionTable.registerBuiltin(-1, testFn)).toThrow(
        'Built-in operations must have indices between 0-127, got -1',
      );
      expect(() => functionTable.registerBuiltin(128, testFn)).toThrow(
        'Built-in operations must have indices between 0-127, got 128',
      );
      expect(() => functionTable.registerBuiltin(1000, testFn)).toThrow(
        'Built-in operations must have indices between 0-127, got 1000',
      );
    });
  });
  ...
  describe('registerBuiltinOpcode', () => {
    ...
    test('should throw error for invalid opcodes', () => {
      ...
      expect(() => functionTable.registerBuiltinOpcode(-1, testFn)).toThrow(
        'Opcode -1 is outside the valid built-in range (0-127)',
      );
      expect(() => functionTable.registerBuiltinOpcode(128, testFn)).toThrow(
        'Opcode 128 is outside the valid built-in range (0-127)',
      );
    });
  });
  ...
  describe('registerWord', () => {
    test('should register user-defined words starting at index 128', () => {
      const testFn1: OpcodeFunction = (vm: VM) => {
        vm.push(1);
      };
      const testFn2: OpcodeFunction = (vm: VM) => {
        vm.push(2);
      };
  ...
  describe('execute', () => {
    ...
    test('should throw error for unregistered function index', () => {
      expect(() => functionTable.execute(vm, 50)).toThrow('No function registered at index 50');
      expect(() => functionTable.execute(vm, 200)).toThrow('No function registered at index 200');
    });

    test('should pass VM instance correctly to functions', () => {
      let capturedVm: VM | null = null;
      const testFn: OpcodeFunction = (vmInstance: VM) => {
        capturedVm = vmInstance;
      };
  ...
  describe('encodeAddress', () => {
    ...
    test('should throw error for invalid addresses', () => {
      expect(() => functionTable.encodeAddress(-1)).toThrow('Invalid function address: -1');
      expect(() => functionTable.encodeAddress(32768)).toThrow('Invalid function address: 32768');
      expect(() => functionTable.encodeAddress(100000)).toThrow('Invalid function address: 100000');
    });
  });
