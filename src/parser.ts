...
export function parse(tokens: (string | number)[]): void {
  ...
  for (const token of tokens) {
    ...
    if (typeof token === "number") {
      ...
      vm.compiler.compileFloat(token); // Use compileInteger for numbers
    } else if (token === "{") {
      ...
      vm.compiler.compileInteger(0); // Placeholder for the relative offset (signed integer)
    } else if (token === "}") {
      ...
      vm.compiler.compileInteger(offset); // Write the relative offset (signed integer)
